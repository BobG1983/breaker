...
| **reviewer-file-length** | Finds oversized files; orchestrator launches sub-agents (background forks) to perform splits in the current branch before merge (never via todo, /implement, or /quickfix); Basic Verification Tier runs after all splits complete |
