...
**Plugin-per-domain** with message-driven decoupling. Each domain plugin (input, breaker, bolt, cells, chips, effect, run, fx, audio, ui, debug) owns its components, resources, and systems. Domains communicate only through Bevy 0.18 messages. See `docs/architecture/` for full details.

## Terminology

All code identifiers MUST use game vocabulary: Breaker (paddle), Bolt (ball), Cell (brick), Node (level), Amp/Augment/Overclock (upgrades), Bump (hit), Flux (currency). No generic terms. See `docs/design/terminology/` for the full glossary.
