...
- Workspace with two crates (game, derive), plugin-per-domain, message-driven decoupling
...
## Debug Domain Structure (implemented, Phase 2f — audited 2026-03-17)
- debug/ restructured into three sub-domains: overlays/ (gizmo drawing), telemetry/ (egui panels), hot_reload/ (RON watching + config/component propagation)
- Hot-reload pipeline: Bevy file_watcher → AssetEvent::Modified → propagate_*_defaults (PropagateDefaults set) → *Config updated → propagate_*_config (PropagateConfig set, resource_changed guard) → entity components updated
...
- Node layout changes mid-play: despawn + re-spawn cells immediately (propagate_node_layout_changes)
- Workspace restructured: single crate now lives in game/ directory (workspace members: game, derive)
...
## Stable Files
- [message-inventory.md](message-inventory.md) — Full message table with ownership, registration, producers, and consumers
