...
- `EffectType::ChainLightning::arc_speed` is a new RON field with `#[serde(default =
...
- `EffectType::TetherBeam::chain` uses `#[serde(default)]` → defaults to `false` when omitted.
...
- New EffectType variants in RON: Anchor, CircuitBreaker, MirrorProtocol, EntropyEngine.
...
- `effects: Vec<RootNode>` — nested via existing RootNode deserialization, which is already vetted.
...
### effect_v3 stamp/route recursive tree walking (Info-level)
- `stamp_effect` and `route_effect` in `EffectCommandsExt` walk `Tree` nodes recursively.
- Recursion depth is bounded by the nesting depth of `Tree::On` within `Tree::On`.
- Shipped RON files use at most 2 nesting levels. No infinite cycle path (each
  `On` consumes its children and dispatches to a resolved entity, not back to the
...
### EffectType::Shield changed from {stacks: u32} to {duration: f32} (Safe)
