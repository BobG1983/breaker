...
- `handle_shockwave` `bolt_query: Query<&Transform>` without Cell/Bolt filter — reads by entity ID, so only the one entity at `trigger.event().bolt` is accessed. No broad scan hazard. However, any entity with Transform would match if queried directly (cosmetic concern, not a logic bug for the happy path).
- Shockwave-written CellDestroyed messages go into next-frame buffer (double-buffered); bridge_overclock_cell_destroyed reads current-frame buffer. No same-frame infinite cascade. Inter-frame cascade (bounded) is possible for OnCellDestroyed(Shockwave) chains.
