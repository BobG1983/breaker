...
- `handle_overclock` observer pushes chain to `ActiveOverclocks` — runs during ChipSelect (OnEnter/FixedUpdate), not during Playing. Chains accumulate across nodes in a single run (correct behavior — you keep chips across nodes within a run). BUG: not cleared between runs (see bug-patterns.md).
- `bypass_menu_to_playing` (scenario runner lifecycle/mod.rs): `Option<ResMut<ActiveOverclocks>>` is always Some at runtime because BoltBehaviorsPlugin registers it via init_resource. Using Option<> is defensive only — not a bug.
