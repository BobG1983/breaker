...
- `bridge_overclock_bump` only arms the specific bolt from `BumpPerformed.bolt` — correct; arming is bolt-specific for multi-bolt (Prism) scenarios.
- `bridge_overclock_impact` evaluates both active chains AND armed triggers for the hit bolt — correct two-level check.
- `bridge_overclock_cell_destroyed` and `bridge_overclock_bolt_lost` evaluate active chains once (global triggers) then evaluate ALL armed bolts — correct because these triggers are not bolt-specific.
...
- `bridge_overclock_cell_destroyed`/`bridge_overclock_bolt_lost`: `armed_query: Query<(Entity, &mut ArmedTriggers)>` without `mut` on parameter binding is valid Rust — Query moves into `evaluate_armed_all` which takes `mut armed_query`. Access type is determined by type parameter, not binding keyword. Do not re-flag.
...
- `bridge_overclock_cell_destroyed`: evaluates once regardless of how many CellDestroyed messages arrived. Same pattern as bolt_lost bridge. Consistent within the codebase.
...
- `bridge_overclock_bump` evaluates BumpSuccess for ALL grades including Perfect — correct; both BumpSuccess and grade-specific triggers (PerfectBump) are independent and can both fire. A chain `OnPerfectBump(leaf)` yields Fire for PerfectBump AND NoMatch for BumpSuccess evaluation. A chain `OnBumpSuccess(leaf)` yields NoMatch for PerfectBump AND Fire for BumpSuccess. No double-firing of same chain.
- `bridge_overclock_bump` BumpSuccess exclusion of whiffs: `BumpPerformed` is only sent for non-whiff bumps (Early, Late, Perfect). Whiffs produce `BumpWhiffed` (a different message type). `bridge_overclock_bump` reads `BumpPerformed` only, so whiffs never reach BumpSuccess evaluation. Correct.
- `bolt_cell_collision` writes BOTH `BoltHitCell` (for overclock bridges) AND `DamageCell` (for handle_cell_hit) on every cell hit — intentional dual-write by design. `handle_cell_hit` consumes DamageCell; `bridge_overclock_cell_impact` consumes BoltHitCell. No duplication in damage application.
...
- `bridge_overclock_wall_impact` ordering: `.after(PhysicsSystems::BreakerCollision)` — BoltHitWall is written by `bolt_cell_collision` which runs before BreakerCollision. BoltHitBreaker written by `bolt_breaker_collision` (runs in BreakerCollision set). `bridge_overclock_breaker_impact` orders after BreakerCollision. All correct — writers precede readers.
