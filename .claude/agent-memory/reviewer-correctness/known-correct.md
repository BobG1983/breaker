...
- Phase 4b.2 (2026-03-19): `apply_bump_velocity` with `BoltSpeedBoost` — floor at `base_speed.0 + boost` and ceiling at `max_speed.0 + boost`. Adding boost to both bounds is intentional: shift the entire valid speed range upward. Correct.
...
- `TriggerChain::depth()` and `is_leaf()` correctly cover all variants including the 7 new ones added in refactor/unify-behaviors (4 leaves: LoseLife, TimePenalty, SpawnBolt, BoltSpeedBoost; 3 triggers: OnEarlyBump, OnLateBump, OnBumpWhiff). Match arms are exhaustive. Do not re-flag.
