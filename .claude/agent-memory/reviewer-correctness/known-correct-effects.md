...
description: Effect system patterns (EffectStack<T>, Phase 4 runtime effects, Phase 5 complex effects) that look suspicious but are intentionally correct
type: project
---

## EffectStack<T> pattern: silent no-op is intentional (post effect_v3 refactor)

`fire()` functions via `fire_dispatch` check `world.get_mut::<EffectStack<T>>()` and silently do nothing if
the component isn't present. There are NO `recalculate_*` systems and NO `Effective*`
components — these were removed in earlier refactors. The effect_v3 system uses
`BoundEffects(pub Vec<(String, Tree)>)` / `StagedEffects(pub Vec<(String, Tree)>)` — `EffectStack<T>` components are only inserted when an
effect's `fire()` actually runs on a bolt or breaker entity.
Consumers call `EffectStack<T>.multiplier()` / `.total()` / `.aggregate()` on demand. The entire system
...
## Multiplicative stacking in EffectStack<T> — correct by design

`EffectStack<DamageBoostConfig>.multiplier()` = product of all entries (not sum). Empty stack
returns 1.0. This is correct for the stated design (additive to multiplicative
...
## dispatch_chip_effects: max-stacks continue is FIXED (old effect system, may be superseded by effect_v3)

`dispatch_chip_effects` had `continue;` after the `add_chip` max-stacks warning.
The old bug (effects dispatched even on max-stack failure) was fixed. Confirmed by test
`chip_at_max_stacks_does_not_dispatch_effects`. NOTE: chip dispatch may have been reworked in effect_v3.
...
## Stat-boost lazy-init: EffectStack<T> in effect_v3

After the effect_v3 refactor, stat effects use `EffectStack<T>` generic component.
`fire_dispatch` routes to per-effect `fire()` functions that lazy-init `EffectStack<T>` if absent.
Do NOT re-flag the absence of `Effective*` insertion — those types were removed long ago.

`quick_stop::fire()` DIFFERS: it does NOT lazy-init `EffectStack<QuickStopConfig>` if absent — it silently
no-ops. This is intentional: QuickStop only applies to entities that already have the component.
However, no gameplay system reads `EffectStack<QuickStopConfig>.multiplier()` for actual deceleration — confirmed open gap.
