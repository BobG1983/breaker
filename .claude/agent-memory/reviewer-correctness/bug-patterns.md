...
## Effect Domain Rewrite Bugs (develop, 2026-03-27) — OPEN

- **init_breaker BoundEffects accumulation when life_pool is None**: `breaker/systems/init_breaker/system.rs:70-93` — The `InitBreakerQuery` filter is `Without<LivesCount>`. When `def.life_pool = None`, `LivesCount` is never inserted, so the entity matches the query on every frame `init_breaker` runs. If scheduled in `OnEnter(GameState::Playing)` (intended location), effects accumulate on every node transition (Playing→TransitionOut→...→Playing). After N nodes, BoundEffects holds N copies of every effect chain. This causes multi-firing of all effects on every trigger. Fix: add a marker component (e.g. `BreakerInitialized`) inserted unconditionally, and filter `Without<BreakerInitialized>` instead of `Without<LivesCount>`. Confidence: HIGH.

- **TransferCommand silently drops non-Do children when BoundEffects/StagedEffects missing**: `effect/commands.rs:89-98` — if the entity exists but lacks `BoundEffects` (when `permanent=true`) or `StagedEffects` (when `permanent=false`), the `get_mut` call returns `None` and non-Do children are silently discarded. No warning is emitted. This is a latent risk if `transfer_effect` is called on entities that don't have both effect components. Confidence: MEDIUM (current callers always target breaker/bolt which have both components; no immediate repro).
