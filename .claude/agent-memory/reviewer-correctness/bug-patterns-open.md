...
## QuickStop: EffectStack<QuickStopConfig> has no consumer reading multiplier — OPEN

`quick_stop::fire()` (now via `fire_dispatch` in effect_v3) manages `EffectStack<QuickStopConfig>` (formerly `ActiveQuickStops`).
Neither `move_breaker` nor `dash::handle_braking` queries `EffectStack<QuickStopConfig>` to
scale their deceleration. `MovementQuery` and `DashQuery` do not include `EffectStack<QuickStopConfig>`.
The `QuickStop` effect fires but its multiplier is never applied to actual deceleration.

**Status**: OPEN — confirmed on cache-removal refactor review 2026-03-30, re-confirmed after effect_v3 refactor.
Location: `effect_v3/effects/quick_stop/` (fire fn), `breaker/queries.rs` (MovementQuery),
...
## WallBuilder dispatch_effects: strips RootNode target, pushes all children to wall entity — LATENT HAZARD

`dispatch_effects` in `terminal.rs` strips the `StampTarget` from `RootNode::Stamp(target, tree)`, discarding
`target`. All tree children are pushed to the wall entity itself, regardless of whether
`target` was `Bolt`, `ActiveCells`, or `EveryWall`.

Current RON (`wall.wall.ron`) has no effects. If a future wall RON adds `Stamp(Bolt, ...)`,
...
**Location**: `breaker-game/src/walls/builder/core/terminal.rs`.
