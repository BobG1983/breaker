...
## Effect Stack Components (effect_v3 EffectStack<T> model)

After the effect_v3 refactor, stat effects use `EffectStack<T>` generic component. Consumers call `.aggregate()` / `.multiplier()` / `.total()` on the stack directly.

- `EffectStack<DamageBoostConfig>` — damage multipliers, `.multiplier()` = product
- `EffectStack<SpeedBoostConfig>` — speed multipliers, `.multiplier()` = product
- `EffectStack<PiercingConfig>` — piercing counts, `.total()` = sum
- `EffectStack<SizeBoostConfig>` — size multipliers, `.multiplier()` = product
- `EffectStack<BumpForceConfig>` — bump force multipliers, `.multiplier()` = product
- `EffectStack<QuickStopConfig>` — deceleration multipliers, `.multiplier()` = product
...
- `speed_boost::fire()` and `reverse()` now call `recalculate_velocity(entity, world)` immediately after push/pop on `EffectStack<SpeedBoostConfig>`.
