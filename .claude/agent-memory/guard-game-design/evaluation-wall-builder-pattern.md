...
2. **`effects: Vec<RootEffect>` on WallDefinition plugs directly into existing effect system.** Target::Wall and Target::AllWalls already exist. Wall-bound effects compose with all chips targeting walls. No new plumbing required for wall-specific chips.
