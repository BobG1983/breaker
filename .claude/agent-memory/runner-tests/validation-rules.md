...
- `MessageReader<'w, 's, E>` takes 2 lifetime parameters. Writing `MessageReader<'w, E>` (only 1 lifetime) triggers E0107. In SystemParam structs use `MessageReader<'w, 'w, E>` (duplicate the world lifetime as the state lifetime).
- `ChipDefinition` in `breaker-game/src/chips/definition/types.rs` intentionally omits `serde::Deserialize` (doc comment says "Never deserialized directly"). Tests in `definition/tests.rs` that call `ron::de::from_str::<ChipDefinition>(...)` will fail to compile (E0277) until `ChipDefinition` derives `Deserialize` or the tests are removed/rewritten. This is a new recurring failure as of branch `feature/seedable-registry`.
