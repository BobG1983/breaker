...
`When(BumpWhiff, Sequence([Once([Fire(DamageBoost(DamageBoostConfig(multiplier: N)))]), Fire(Shockwave(...))]))`
...
NOTE: RON syntax uses effect_v3 `Tree` variants (`Fire`, `When`, `Once`, `Sequence`) not old `Do`/`EffectNode`.
