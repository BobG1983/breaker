...
- `seed()` clones both `def.name` (String) and `def` (WallDefinition including `Vec<RootNode>`) per entry — this is startup-only, not per-frame
...
- `WallDefinition` stores `Vec<RootNode>` — heap allocation per definition, but these are owned by the registry (a Resource), not attached to entities
