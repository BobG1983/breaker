...
- **`def.clone()`**: Clones the BreakerDefinition once. Definition is a plain data struct with no heap allocations beyond `Vec<RootEffect>`. Acceptable at fire-once frequency.
