...
**Why:** `unimplemented!()` and `todo!()` both panic, but `todo!()` is the idiomatic "I haven't written this yet" marker. The stub needs to compile fully (including the workspace `missing_docs` and `clippy::pedantic` lint requirements) before the code-writer implements it.
