...
**Wall hit logic:**
- Reflects velocity off hit normal
- Resets `PiercingRemaining` to `Piercing.0`
- Sends `BoltImpactWall { bolt, wall }`
...
- `BoltImpactWall { bolt: Entity, wall: Entity }` — consumed by bolt/behaviors (overclock wall impact bridge)

---

### 2. `bolt_wall_collision` — STUB

**File:** `src/bolt/systems/bolt_wall_collision.rs`
**Schedule:** `FixedUpdate`, run_if `PlayingState::Active`
**Ordering:** after `BoltSystems::CellCollision`

Empty stub. Wall collision physics live entirely in `bolt_cell_collision`'s unified CCD sweep. Registered for forward compatibility with per-collision-type architecture.
...
The bolt-wall collision logic is NOT in the `bolt_wall_collision` system (which is a stub). It lives entirely inside `bolt_cell_collision` because cells and walls share the quadtree CCD sweep. The `bolt_wall_collision` stub exists for forward compatibility only.

The four AABB-overlap systems (breaker-cell, breaker-wall, cell-wall) are all described as "placeholders" for future moving-entity mechanics. The only currently active gameplay collision is the `bolt_cell_collision` + `bolt_breaker_collision` pair.
