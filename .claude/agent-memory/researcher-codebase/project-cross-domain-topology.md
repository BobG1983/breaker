...
- **bolt ↔ effect**: tight cycle — effect effects spawn/modify bolts (spawn_bolts, gravity_well); bolt uses effect component types (BoundEffects, EffectStack<SpeedBoostConfig>, etc.).
...
- **effect_v3 types** (RootNode, BoundEffects, StagedEffects, Tree, StampTarget) appear in BoltDefinition, BreakerDefinition, CellTypeDefinition, ChipDefinition — so all four definition-holding domains depend on effect_v3 for their own data types.
