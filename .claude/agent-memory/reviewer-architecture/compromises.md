...
- screen/run_setup reads breaker::BreakerRegistry (was behaviors::ArchetypeRegistry before C7-R; moved to breaker/registry.rs)
...
- breaker/systems/init_breaker.rs (was behaviors/init.rs before C7-R) writes ResMut<BreakerConfig> and inserts breaker-owned components at init time — accepted for archetype config composition
- effect/plugin.rs (was behaviors/plugin.rs before C7-R) orders against BreakerSystems::InitParams and UiSystems::SpawnTimerHud
- effect/effects/life_lost.rs (was behaviors/effects/life_lost.rs before C7-R) reads ui::StatusPanel (read-only, for HUD parenting)
...
- **chips/apply_chip_effect writes ResMut<ActiveEffects>**: chips domain pushes triggered chains to effect domain's `ActiveEffects` resource. Same class as breaker/systems/init_breaker.rs (was behaviors/init.rs before C7-R) writing ResMut<BreakerConfig>. Pre-existed B1-B3 refactor.
...
- ~~bolt/behaviors/effects/shockwave.rs cross-domain mutation~~ → FIXED 2026-03-20 (feature/overclock-trigger-chain): shockwave now writes `DamageCell` messages (consumer-owns pattern); cells/handle_cell_hit processes damage. No direct CellHealth mutation. NOTE: shockwave.rs now lives at effect/effects/shockwave.rs (bolt/behaviors/ deleted in refactor/unify-behaviors; behaviors/ renamed to effect/ in C7-R).
...
- ~~effect/plugin.rs (was behaviors/plugin.rs before C7-R) uses bare fn refs for cross-domain OnEnter ordering~~ → BreakerSystems::InitParams + UiSystems::SpawnTimerHud extracted
