...
- **Full spatial/physics extraction audit 2026-03-24**: PASS — 0 BLOCKING, 1 IMPORTANT, 3 MINOR, 2 doc drift. rantzsoft_spatial2d (6 crate members) and rantzsoft_physics2d (quadtree, CCD, CollisionLayers, DistanceConstraint) fully reviewed. Zero game vocabulary in either crate. Plugin boundaries correct. Old interpolation domain and old game-level physics/ domain fully removed. #[require] patterns correct on all entity markers (Bolt, Breaker, Wall, Cell). No game systems write Transform directly. IMPORTANT: derive_transform + propagate_* dual pipeline is redundant — propagate_* overwrites Transform.translation last in the chain. Doc drift: ordering.md still references old BoltSystems::BreakerCollision/BoltLost (now BoltSystems variants).

- **B12c typed events audit 2026-03-24**: 2 BLOCKING, 3 IMPORTANT, 5 MINOR. behaviors/ renamed to effect/ — source code fully updated, docs/architecture/ NOT updated (BLOCKING-1). typed_events.rs non-canonical file (BLOCKING-2). Dual type system TriggerChain vs Trigger/Effect/EffectNode is transitional (IMPORTANT-1). EffectFired/ChipEffectApplied dead code retained for test compat (IMPORTANT-2). Passive event types in wrong domain — effect/ instead of chips/ (IMPORTANT-3). Bridge dispatch pattern (fire_typed_event) is correct. 13 triggered handlers + 10 passive handlers all observe typed events. Plugin registration order correct. EffectSystems::Bridge exported correctly.
