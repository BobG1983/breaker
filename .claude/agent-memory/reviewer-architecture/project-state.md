...
- **Overclock trigger chain audit 2026-03-20**: BLOCKING — 1 cross-domain mutation violation. bolt/behaviors/effects/shockwave.rs directly mutates cells-domain CellHealth, despawns cell entities, and writes CellDestroyed message. Must be refactored to message-based indirection (same pattern as behaviors/consequences). Module organization, observer pattern, visibility changes, scenario runner integration, and bridge system ordering all PASS. 3 doc drift items (OverclockEffectFired missing from messages.md, bridge ordering missing from ordering.md, ActiveOverclocks/ArmedTriggers undocumented).
