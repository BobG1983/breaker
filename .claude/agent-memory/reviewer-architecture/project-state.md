...
- **Phase 4b.2 effect consumption audit 2026-03-19**: PASS with 1 medium observation. chips/ components (Piercing, PiercingRemaining, DamageBoost, BoltSpeedBoost, BreakerSpeedBoost, WidthBoost, TiltControlBoost, BumpForceBoost) read by physics, cells, breaker, bolt domains. All read-only except PiercingRemaining (mutated by physics for collision response — same class as BoltVelocity mutation). width_boost_visual system added to breaker in Update schedule (visual-only). Stacking infra in chips/effects/mod.rs (stack_u32, stack_f32). Observer dispatch via ChipEffectApplied. Medium: TiltControlBoost test at bolt_breaker_collision.rs:700 likely has double-radians-conversion bug. Low: apply_bump_velocity doesn't account for BoltSpeedBoost in clamp. Info: BoltSizeBoost and ChainHit stamped but not yet consumed by production systems.
