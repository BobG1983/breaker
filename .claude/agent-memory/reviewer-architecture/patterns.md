...
- ~~interpolate/ domain~~: DEAD CODE as of 2026-03-23. InterpolatePlugin still registered in game.rs but all systems are no-ops (no entities have InterpolateTransform/PhysicsTranslation). Pending removal. Old pattern: FixedFirst (restore_authoritative), FixedPostUpdate (store_authoritative), PostUpdate (interpolate_transform). Entities opted in via InterpolateTransform + PhysicsTranslation components.
