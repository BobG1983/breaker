...
- **B1-B3 TriggerChain flattening** (2026-03-24): ChipEffect enum removed. All chip effects now expressed as TriggerChain variants. apply_chip_effect dispatches three ways: OnSelected → fires ChipEffectApplied per leaf; bare leaf → fires ChipEffectApplied directly; triggered chain → pushes to ActiveEffects (cross-domain write to effect resource, accepted compromise). handle_overclock observer DELETED — replaced by direct push in apply_chip_effect. TriggerChain trigger wrappers use Vec<Self> (not Box<Self> as content.md shows).
