...
- `loading/` owns `build_chip_catalog` system — post-seed ChipCatalog construction from ChipTemplateRegistry + EvolutionRegistry. Config seeding is now handled by `RantzDefaultsPluginBuilder` in `ScreenPlugin`, not by `LoadingPlugin` systems.
...
- **rantzsoft_spatial2d domain** (2026-03-23; updated 2026-03-26): Canonical position is `Position2D` (from rantzsoft_spatial2d). `RantzSpatial2dPlugin<GameDrawLayer>` registered in game.rs. Systems: FixedFirst (save_previous in SpatialSystems::SavePrevious), FixedUpdate (apply_velocity in SpatialSystems::ApplyVelocity), RunFixedMainLoop/AfterFixedMainLoop (compute_globals in SpatialSystems::ComputeGlobals, then derive_transform in SpatialSystems::DeriveTransform — chained). `propagate_position/rotation/scale` NOT registered by plugin. Entities opt in via `Spatial2D` marker (#[require]s Position2D, Rotation2D, Scale2D, PreviousPosition, PreviousRotation, PositionPropagation, RotationPropagation, ScalePropagation). Add `InterpolateTransform2D` for interpolated rendering. Add `GameDrawLayer` variant for Z ordering. `GameDrawLayer` defined in shared/draw_layer.rs (passive type). `SpatialSystems` set exported from `rantzsoft_spatial2d::plugin` and prelude — use for game system ordering relative to spatial pipeline.
...
- **C7-R chip dispatch** (2026-03-25): `apply_chip_effect` + `ChipEffectApplied` DELETED. Replaced by `dispatch_chip_effects` in `chips/systems/dispatch_chip_effects.rs`. Dispatch: `ChipSelected` message → `dispatch_chip_effects` → pushes EffectNode trees to entity `EffectChains` + fires passive typed events (PiercingApplied, DamageBoostApplied, etc.) via `fire_passive_event()`. TriggerChain flattening is historical — current model uses `EffectNode` trees directly.
