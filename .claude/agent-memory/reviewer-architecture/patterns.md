...
- **behaviors→effect domain rename** (2026-03-24, B12): `behaviors/` renamed to `effect/`. BehaviorsPlugin→EffectPlugin. BehaviorSystems→EffectSystems. ActiveChains→ActiveEffects. ArmedTriggers→ArmedEffects. B12c: EffectFired catchall replaced by per-effect typed events (ShockwaveFired, LoseLifeFired, etc.) via fire_typed_event(). New split type system: Trigger/Effect/EffectNode enums (effect/definition.rs) parallel to TriggerChain (chips/definition.rs) — transitional. trigger_chain_to_effect() converts between them. EffectFired and ChipEffectApplied retained for test compat only. ActiveEffects pub for scenario runner.
