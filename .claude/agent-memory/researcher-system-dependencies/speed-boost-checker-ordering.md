...
description: One-frame lag between ActiveSpeedBoosts update and bolt speed — root cause and the two-option fix
type: project
---

## Root Cause: `ActiveSpeedBoosts` Updated After `apply_velocity_formula` Runs
...
variant of Option B (velocity recalculated after ActiveSpeedBoosts changes). However, the
...
so the checker sees `ActiveSpeedBoosts` updated to the new multiplier while the
...
FixedUpdate:
  ...
  2. EffectSystems::Bridge         — bridge_impact_* runs, queues FireEffectCommand
     [command flush after bridge]  — FireEffectCommand::apply() pushes onto
                                      ActiveSpeedBoosts (NOW UPDATED)
  3. checkers_a chain              — check_bolt_speed_accurate reads new ActiveSpeedBoosts,
...
This is the "ground-truth" fix: bolt speed is always consistent with `ActiveSpeedBoosts`
