...
description: Scenario and invariant gaps introduced by the bolt builder migration — init_bolt_params deleted, spawn_extra_bolt removed, steering model for attraction/gravity well, BreakerReflectionSpread renamed
type: project
---
...
| BoltInitialAngle / BoltRespawnAngleSpread via config() | Only unit tests (launch_bolt.rs) | NO scenario verifies angle-spread distribution |
...
2. **BoltConfigParams injection via config() — no scenario probes component presence** — if the builder's `config()` path fails to insert `BoltRespawnOffsetY`/`BoltRespawnAngleSpread`/`BoltInitialAngle`, `bolt_lost` would panic/skip on those fields. No invariant catches missing required bolt components on spawn.
