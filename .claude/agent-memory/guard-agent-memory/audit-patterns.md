...
## Audit History
2026-03-26 (targeted: SeedableRegistry feature) — 7 agent memory files audited, 9 edits. Key changes: DefaultsCollection DELETED + 14 seed systems DELETED (replaced by RantzDefaultsPluginBuilder.add_config + add_registry). SeedableRegistry trait added to rantzsoft_defaults::prelude (alongside SeedableConfig). RegistryHandles<A> resource added. ChipRegistry (old name) → ChipTemplateRegistry + EvolutionRegistry (both now SeedableRegistry). ChipCatalog is a derived resource built by build_chip_catalog (NOT SeedableRegistry). Config structs now use reversed GameConfig form (defaults = "Foo", path, ext). Agents fixed: researcher-codebase (defaults-config-pipeline.md REWRITTEN, rantzsoft-crate-exports.md updated with SeedableRegistry/RegistryHandles), researcher-system-dependencies (system-map.md seeding section replaced, architectural-facts.md hot-reload + seed_chip_registry), guard-docs/known-state.md (DefaultsCollection refs x2, ChipEffectApplied/ChipRegistry, rantzsoft_defaults description, ChipEffectApplied table note), reviewer-architecture (patterns.md loading/ description + B1-B3 TriggerChain description, message-inventory.md HighlightTriggered BLOCKING note resolved), planner-spec (domain-inventory.md BASE_BOLT_DAMAGE location).

2026-03-26 (full audit, post-C7-R chip-effects wave, part 2) — Continued from part 1. Additional files fixed: runner-linting/lint_patterns.md SPLIT into lint_patterns_core.md (durable patterns, ~80 lines) + lint_patterns_history.md (session-dated log, ~180 lines) + lint_patterns.md redirect stub — was 442 lines (3x the 150-line threshold). planner-spec/domain-inventory.md: 6 stale items updated (ActiveEffects deleted, ArmedEffects now holds EffectNode not TriggerChain, EffectChains is implemented not planned, evaluate_node signature updated, fire_leaf deleted, TriggerChain test constructors deleted). reviewer-architecture/message-inventory.md: ChipEffectApplied DELETED, dispatch_chip_effects replaces apply_chip_effect; passive typed events row added. researcher-system-dependencies/system-map.md: EffectPlugin section rewritten (ActiveEffects/TriggerChain/BehaviorSystems::Bridge all stale). researcher-system-dependencies/known-conflicts.md: 5 stale references fixed (propagate_archetype_changes→propagate_breaker_changes; BehaviorSystems::Bridge→EffectSystems::Bridge; EffectFired/TriggerChain references). researcher-system-dependencies/architectural-facts.md: hot-reload pipeline entry updated (ActiveEffects→EffectChains component). researcher-system-dependencies/message-flow.md: 7 stale entries fixed (TriggerKind/EffectFired references, ChipEffectApplied/apply_chip_effect, ArmedEffects now uses EffectNode). reviewer-architecture/ordering-chain.md: apply_archetype_config_overrides→apply_breaker_config_overrides.

2026-03-26 (full audit, post-C7-R chip-effects wave, part 1) — 28 agent directories audited. 7 files fixed. Key changes: TypeId panic (chip_templates path) RESOLVED in source — removed stale "Active Known Flaky Failures" section from runner-scenarios/known_invariant_false_positives.md. Known Failing Unit Tests section removed (tests were updated). TriggerChain/ActiveChains/BehaviorSystems stale references pruned from reviewer-correctness/bug-patterns.md (refactor/unify-behaviors section collapsed; init_archetype bug marked resolved). guard-docs/known-state.md: chips domain architecture updated (TriggerChain/ChipEffectApplied deletions), TriggerChain Unification + SpeedBoost sections condensed, recurring drift pattern updated. researcher-impact/MEMORY.md: ephemeral file listing removed from stable section (recurring issue, noted in audit-patterns.md).

2026-03-26 (targeted: rantzsoft_* prelude + SeedableConfig + cast_circle session) — 8 agent directories audited, 12 files fixed. Key changes: rantzsoft_spatial2d added `prelude` module and `SpatialSystems` set (4 variants: SavePrevious, ApplyVelocity, ComputeGlobals, DeriveTransform). `propagate_position/rotation/scale` confirmed NOT registered by plugin — only compute_globals + derive_transform are active. rantzsoft_physics2d `ccd` module is pub(crate) — CCD types (RayHit, SweepHit) access via prelude only. `SweepHit` type added, `cast_circle` method added to Quadtree. rantzsoft_defaults expanded: `SeedableConfig` trait, reversed `GameConfig` macro form (defaults = "..."), `merge_from_defaults` method, `DefaultsHandle`, `RonAssetLoader`, `DefaultsSystems`, `RantzDefaultsPlugin`/`Builder` — all in new plugin/handle/loader/seedable modules and prelude. Game codebase still uses its own DefaultsCollection pipeline; rantzsoft_defaults plugin pipeline not yet wired in game. Agents fixed: researcher-codebase (rantzsoft-crate-exports.md, defaults-config-pipeline.md, spatial2d-propagation-flow.md, MEMORY.md), researcher-system-dependencies (system-map.md, architectural-facts.md), planner-spec (domain-inventory.md), reviewer-architecture (patterns.md, ordering-chain.md), reviewer-quality (intentional-patterns.md), reviewer-bevy-api (confirmed-patterns.md).
...
2026-03-24 (targeted: spatial/physics extraction refactor) — 14 agent memory files audited. 30+ stale references fixed across 14 files. Key changes: physics/ game domain DELETED, interpolate/ game domain DELETED. BoltPlugin absorbs all collision systems (bolt_cell_collision, bolt_breaker_collision, clamp_bolt_to_playfield, bolt_lost) and their messages (BoltHitBreaker, BoltHitCell, BoltHitWall, BoltLost now in bolt/messages.rs). PhysicsSystems enum DELETED (replaced by BoltSystems::BreakerCollision, BoltSystems::BoltLost). PhysicsTranslation + InterpolateTransform components DELETED; Position2D canonical position from rantzsoft_spatial2d. interpolate_transform/restore_authoritative/store_authoritative systems DELETED. Agents fixed: researcher-system-dependencies (architectural-facts.md, known-conflicts.md, system-map.md, message-flow.md), reviewer-architecture (ordering-chain.md, message-inventory.md, patterns.md, compromises.md), reviewer-bevy-api (confirmed-patterns.md), researcher-codebase (bolt-boundary-and-bump-flow.md, message-flow-for-run-stats.md), reviewer-performance (performance-baseline.md), planner-spec (domain-inventory.md), runner-linting (lint_patterns.md), planner-review (MEMORY.md domain quirks).

### Loading pipeline overhaul cascades across all memory files (SeedableRegistry feature, 2026-03-26)
When the loading pipeline is replaced wholesale (e.g., `DefaultsCollection` + 14 seed systems → `RantzDefaultsPluginBuilder`), every agent that documented the old pipeline holds stale data: system-map.md (ScreenPlugin seeding section), architectural-facts.md (hot-reload counts, orphan system references), known-conflicts.md (hot-reload systems), defaults-config-pipeline.md (complete rewrite), rantzsoft-crate-exports.md (missing new types), known-state.md (DefaultsCollection references, ChipRegistry name), reviewer-architecture patterns.md (old dispatch patterns), reviewer-correctness bug-patterns.md (old system names).
**Watch:** When `RantzDefaultsPluginBuilder` gains new capabilities (e.g., new trait methods, hot-reload integration), update: `defaults-config-pipeline.md`, `rantzsoft-crate-exports.md`, `architectural-facts.md` hot-reload section.
**Fast check:** Search agent memories for `DefaultsCollection`, `AssetCollection`, `load_collection`, `seed_chip_registry`, `seed_archetype_registry`, `ChipRegistry` (old name), `bevy_asset_loader` — all should now return zero hits.
...
Observed again: spatial/physics extraction (2026-03-24) deleted physics/ game domain and interpolate/ game domain entirely. PhysicsPlugin, InterpolatePlugin, PhysicsSystems, PhysicsTranslation, InterpolateTransform, restore_authoritative, store_authoritative, interpolate_transform all deleted. Stale references appeared in 14 memory files across 8 agent directories.
**Watch:** When any subdomain is deleted, immediately audit: planner-spec/domain-inventory.md, researcher-system-dependencies (all 4 files), reviewer-architecture (all 5 files), reviewer-correctness, reviewer-quality, reviewer-performance, reviewer-bevy-api, runner-linting, guard-docs (terminology, known-state, phase-log), guard-game-design, researcher-codebase.
**Pattern:** Grep all agent memories for the old subdomain path (e.g., `bolt/behaviors`, `behaviors/consequences`, `physics/`, `interpolate/`) and old type names before closing the branch.
**Fastest grep:** `grep -r "PhysicsPlugin\|InterpolatePlugin\|PhysicsSystems\|PhysicsTranslation\|InterpolateTransform[^2]" .claude/agent-memory/` — catches the deleted game-domain types without false-positives on rantzsoft_physics2d's `PhysicsSystems::MaintainQuadtree`.
...
- `runner-linting/lint_patterns.md` — SPLIT 2026-03-26 (was 442 lines). Now: `lint_patterns_core.md` (~80 lines, durable patterns) + `lint_patterns_history.md` (~180 lines, session log). lint_patterns.md is now a redirect stub. Revisit if either new file approaches 150 lines.
