...
- **runner.rs grouping helpers**: group_violations covered (same-kind grouping, different-kind separation, single entry). Known gaps: (1) `group_logs` has no test for two entries with same message but different levels — should be two groups, not one. (2) `is_invariant_fail_reason` and `is_health_check_reason` have no unit tests; `is_invariant_fail_reason` maintains a hard-coded VARIANTS slice that must stay in sync with InvariantKind — needs a test asserting every fail_reason() string returns true and a random string returns false.
