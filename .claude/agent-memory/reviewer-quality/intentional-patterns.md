...
- `track_node_cleared_stats.rs` uses 7 items from `crate::run::resources` without a glob — the 4+ glob rule applies. Should use `use crate::run::resources::*`. Flag.
...
- `MassDestruction` and `FirstEvolution` in `HighlightKind` — defined in resources.rs but no system generates them in Wave 3. These are planned variants, not bugs; but they represent a coverage gap (defined but never emitted).
