...
- Per-effect observer files (`bolt_speed_boost.rs`, `chain_hit.rs`, `bolt_size_boost.rs`, `breaker_speed_boost.rs`, `bump_force_boost.rs`, `tilt_control_boost.rs`) each have only one happy-path test (insert-only). Stacking and cap tests exist only in `piercing.rs`, `width_boost.rs`, and `damage_boost.rs`. This coverage gap was noted in coverage-standards.md as of 2026-03-19 and is still open.
