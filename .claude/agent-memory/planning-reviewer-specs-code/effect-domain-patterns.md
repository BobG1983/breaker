...
When a code spec proposes push/pop into a stat stack component (e.g., `EffectStack<BumpForceConfig>`, formerly `ActiveBumpForces`) from within a Bevy SYSTEM (not from a `fire()`/`reverse()` function that takes `&mut World`):

1. **The system cannot use `world.get_mut()` directly** — systems don't take `&mut World` as a parameter.
2. **Commands cannot push/pop** — `commands.entity(e).insert(EffectStack::<BumpForceConfig>::default())` REPLACES the component, destroying existing entries from other effects.
3. **The correct approach** is to add `Option<&'static mut EffectStack<BumpForceConfig>>` to the system's query tuple, then mutate the stack in-place via the query field. NOTE: `EffectiveBumpForce` and `ActiveBumpForces` were removed — use `EffectStack<BumpForceConfig>.multiplier()` on demand instead.
4. **Lazy-insert when absent**: If the query field is `None`, use `commands.entity(e).insert(EffectStack::<BumpForceConfig>::default())` only when no other effects have contributed — but this still loses existing values. The robust approach is to require `EffectStack<BumpForceConfig>` be always present on breaker entities (initialized at spawn), avoiding the conditional insert entirely.

Any code spec that says "push into `EffectStack<BumpForceConfig>` in `tick_anchor`" MUST specify that `AnchorTickQuery` is extended to include `Option<&'static mut EffectStack<BumpForceConfig>>`. Without this, the implementation has no way to read-modify-write the stack. Flag as BLOCKING if missing.

Also check import paths: effect modules are now in `effect_v3/effects/`.
...
## Dispatch Function in effect_v3

`fire_dispatch()` and `reverse_dispatch()` are free functions in `effect_v3/dispatch/`. They match on `EffectType` / `ReversibleEffectType` and route to per-effect `fire()` / `reverse()` functions. NOTE: the old `EffectKind::fire()` split across 4 sub-methods is superseded.

**Rule:** Code specs updating dispatch must name the exact module/function — not just "the match arm."
