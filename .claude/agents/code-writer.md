---
name: code-writer
description: "Use this agent to implement production code that satisfies existing failing tests. The code-writer is the GREEN phase of the TDD cycle — it receives failing tests written by the test-writer and implements the minimal code to make them pass. Always used as the second half of the test-writer → code-writer pair, after the main agent reviews the tests.\n\nExamples:\n\n- After the main agent reviews test-writer output:\n  Assistant: \"Tests look correct. Let me use the code-writer agent to implement the code that satisfies them.\"\n\n- When delegating domain implementation:\n  Assistant: \"Launching code-writers for bolt and cells domains in parallel — each has failing tests to satisfy.\"\n\n- After test review checkpoint:\n  Assistant: \"All test specs approved. Let me use the code-writer to make them pass.\""
