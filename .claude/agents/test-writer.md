---
name: test-writer
description: "Use this agent to write failing tests from a behavioral spec before implementation begins. The test-writer translates behavioral descriptions into concrete Rust/Bevy test code that compiles but fails, establishing the TDD red phase. Always used as the first half of the test-writer → code-writer pair. The main agent reviews test output before launching the code-writer.\n\nExamples:\n\n- Before implementing a new system:\n  Assistant: \"Let me use the test-writer agent to create failing tests from this behavioral spec.\"\n\n- When delegating domain implementation:\n  Assistant: \"Launching test-writers for bolt and cells domains in parallel — each gets a behavioral spec.\"\n\n- After the main agent writes a behavioral spec:\n  Assistant: \"Spec ready. Let me use the test-writer to translate this into failing tests.\""
