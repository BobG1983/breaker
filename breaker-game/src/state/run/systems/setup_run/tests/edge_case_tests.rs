...
fn setup_run_reads_bolt_name_from_breaker_definition() {
    ...
    let test_breaker_def: BreakerDefinition = ron::de::from_str(
        r#"(name: "TestBreaker", bolt: "HeavyBolt", life_pool: Some(3), bolt_lost: Stamp(Breaker, When(BoltLostOccurred, Fire(LoseLife(())))), salvo_hit: Stamp(Breaker, When(Impacted(Salvo), Fire(LoseLife(())))), effects: [])"#,
    )
