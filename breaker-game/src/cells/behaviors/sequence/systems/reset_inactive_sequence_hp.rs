...
//!
//! Runs in `FixedUpdate`, ordered `.after(DmgSystems::ApplyDamage)`
//! and `.before(DmgSystems::EmitKill)`. For every sequence cell
...
//! AND clears `KilledBy.killer` to `None`.
//!
...
//! and clearing `KilledBy.killer` even in the absence of any damage.
//!
//! The `KilledBy.killer` clear is essential: `apply_damage::<Cell>` stamps
//! `killed_by.killer = msg.dealer` gated on `is_none()` — if the reset system
//! only restores HP without clearing the killer, a future legitimate killing
//! blow against the cell (once it becomes active) is silently rejected by
//! the `is_none()` gate, latching stale killer state across the reset. The
...
/// Restores `hp.current` to the healing ceiling and clears `killed_by.killer`
...
/// `starting`. Clearing `KilledBy.killer` is required because `apply_damage`
/// stamps it on the killing blow via an `is_none()` gate; a stale killer
