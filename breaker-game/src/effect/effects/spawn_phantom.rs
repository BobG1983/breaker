...
///
/// Kept only for backward compatibility so tests can assert its absence.
/// Not spawned on new phantom bolts — `BoltLifespan` is used instead.
#[derive(Component)]
...
mod tests {
    ...
    #[test]
    fn fire_spawns_phantom_with_bolt_lifespan_not_phantom_timer() {
        let mut world = world_with_bolt_config();
        let entity = world.spawn(Position2D(Vec2::ZERO)).id();
        ...
        // Should NOT have PhantomTimer
        assert!(
            world.get::<PhantomTimer>(phantom).is_none(),
            "phantom should NOT have PhantomTimer (replaced by BoltLifespan)"
        );
    }
    ...
    fn fire_enforces_max_active_cap() {
        ...
        let mut bolt_query =
            world.query_filtered::<Entity, (With<PhantomBoltMarker>, With<Bolt>)>();
