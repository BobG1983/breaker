...
//! `afterimage_spawn_phantom_bolt`), and the `activate` / `register`
...
//! `EffectV3Plugin` via `SpawnPhantomConfig::register`.
//!
...
// and `register`. The test-only re-exports are gated with `#[cfg(test)]` so
...
// re-exported because nothing consumes them by that path (register.rs uses
