...
//! `starting_hp − (base × boost²) − (base × cashout × boost)`. The
//! double-apply term collapses to a single application once W6 lands.
...
fn debt_collector_on_impact_applies_damage_boost_in_same_tick() {
    ...
    //     = 200.0 − (10.0 × 4.0) − (10.0 × 2.0 × 2.0)
    //     = 200.0 − 40.0 − 40.0
    //     = 120.0
    assert!(
        (hp - 120.0).abs() < 1e-5,
        "final_hp = 120.0 (baseline 40.0 + debt 40.0 dropped from 200.0), got {hp}"
    );
}
