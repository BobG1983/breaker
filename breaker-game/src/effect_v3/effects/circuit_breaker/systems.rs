...
use crate::{
    ...
    effect_v3::{dispatch::fire_dispatch, effects::ShockwaveConfig, types::EffectType},
};

/// Processes `BumpPerformed` messages to decrement circuit breaker counters.
///
/// When a counter reaches zero, fires reward effects (shockwave + spawn bolts)
/// via `fire_dispatch` and resets the counter. Processes bumps sequentially
/// per entity (fire-reset-continue within frame).
///
/// Exclusive system — requires `&mut World` for `fire_dispatch`.
pub fn tick_circuit_breaker(world: &mut World) {
    ...
    // Phase 2: Collect counter entities and their data (immutable borrow ends here).
    let entries: Vec<(Entity, CircuitBreakerCounter)> = {
        let mut query = world.query::<(Entity, &CircuitBreakerCounter)>();
    ...
    for (entity, mut counter) in entries {
        for _ in 0..bump_count {
            ...
            if counter.remaining == 0 {
                // Fire reward shockwave.
                let shockwave = EffectType::Shockwave(ShockwaveConfig {
                ...
                fire_dispatch(&shockwave, entity, "", world);
        ...
        if let Some(mut c) = world.get_mut::<CircuitBreakerCounter>(entity) {
            c.remaining = counter.remaining;
        }
    }
}
...
mod tests {
    ...
    use crate::{
        ...
        effect_v3::effects::{
            circuit_breaker::components::CircuitBreakerCounter,
