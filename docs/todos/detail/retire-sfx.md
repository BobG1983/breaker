...
Sweep `docs/design/protocols/*.md` and `docs/design/hazards/*.md`. In each file:
...
Today only `conductor.md` has an SFX mention (confirmed by grep of `docs/design/protocols/` and `docs/design/hazards/`). If future audits surface additional files, they get the same treatment in the same commit.
