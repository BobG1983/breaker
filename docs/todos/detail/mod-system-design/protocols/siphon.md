...
- **What it does**:
  1. Reads `Destroyed<Cell>` messages.
...
- **Ordering**: After cell destruction systems (so `Destroyed<Cell>` has been sent). Before `NodeSystems::ApplyTimePenalty` (so added time is factored into the same frame's timer tick).
...
- **cells**: Reads `Destroyed<Cell>` message. All kill sources (bolt, AoE, chain lightning, explosions) already funnel through the cell destruction pipeline and emit this message.
...
1. **First kill starts streak but adds no time**
   ...
   - When: `Destroyed<Cell>` is sent (first kill)
...
2. **Second kill within window adds time**
   ...
   - When: `Destroyed<Cell>` is sent (second kill)
...
3. **Third and subsequent kills each add time**
   ...
   - When: `Destroyed<Cell>` is sent (third kill)
...
7. **Multiple kills in same frame each count**
   ...
   - When: 3 `Destroyed<Cell>` messages processed in sequence
...
## Edge Cases
- **AoE destroying many cells at once**: Each destruction event counts as a separate kill. A shockwave killing 8 cells produces 8 `Destroyed<Cell>` messages. The first starts/continues the streak, subsequent ones each add time (if streak is active from a prior kill or the first of this batch).
