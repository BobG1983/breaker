...
- **Siphon**: `SiphonStreak` resource, reads `Destroyed<Cell>`, sends `ReverseTimePenalty` — [protocols/siphon.md](protocols/siphon.md)
...
- Scenario-runner backdoors (2026-04-17): `MutationKind::InjectHazardStack { kind_name, stacks }` and `MutationKind::InjectProtocol { kind_name }`, both routed through new pub helpers `hazard::activate_from_registry` and `protocol::activate_from_registry`. Lets scenarios install a hazard or protocol without going through the chip-select / hazard-select UI, unlocking install-path scenario coverage for every variant.
- Smoke scenarios (2026-04-17): every protocol (15) and every hazard (16) now has at least one smoke scenario — `decay_stack_3_smoke`, `deadline_smoke`, etc. — that activates the variant and runs 500 frames asserting NoNaN, BoltInBounds, BreakerInBounds, TimerNonNegative, and either BoltSpeedAccurate (protocols) or HazardStackValid (hazards). Catches infrastructure regressions even where the runtime body is still a stub.
- Scaffold log levels (2026-04-17): per-variant "X activated — Y plumbing pending" stub messages downgraded from `warn!` to `info!`. The scenario runner captures WARN+ERROR as scenario failures; info-level keeps the diagnostic visible in dev without failing scenarios during the staged rollout.
- Wave 5 runtime behaviour — Conductor (2026-04-20, branch `feature/protocol-conductor`): `conductor_swap_on_perfect_bump` in FixedUpdate swaps `PrimaryBolt`/`ExtraBolt` markers and `BoundEffects`/`StagedEffects` components between the bumped `ExtraBolt` and the current `PrimaryBolt` on every Perfect bump. Single system; no grace window; no bolt-lost suppression (dropped in design pivot — any bolt lost still costs a life). `ConductorConfig` is a unit struct (presence marker only — `primary_swap_window` removed 2026-04-20). Ordering: `.after(BreakerSystems::GradeBump)`, `.before(EffectV3Systems::Bridge)` — ensures the swap precedes bridge systems that walk the effect tree on the newly promoted bolt.
- Wave 5 runtime behaviour — Fission (branch `feature/protocol-burnout`): single system `fission_on_cell_destroyed` reads `Destroyed<Cell>` messages, increments `FissionCounter` (global Resource persisting across nodes), and spawns a new bolt via `Bolt::builder()` when `kills >= kills_per_split`. New bolt velocity is parent velocity rotated by `FISSION_DIVERGENCE_ANGLE_RAD` (15°). Ordering: `.after(DeathPipelineSystems::HandleKill)`. Gated by `protocol_active(Fission)` + `in_state(NodeState::Playing)`. `FissionCounter` cleared on `OnExit(MenuState::Main)` alongside `FissionConfig`.
- Wave 5 runtime behaviour — Afterimage (branch `feature/protocol-burnout`): 4 systems — `afterimage_tick_phantom_breaker` (decrements `PhantomBreakerLifetime`, despawns on expiry), `afterimage_spawn_phantom_breaker` (on dash start: spawns phantom at pre-dash position; despawns old phantom first), `afterimage_check_phantom_bounce` (re-emits `BoltImpactBreaker` when bolt contacts phantom; runs `.before(BreakerSystems::GradeBump).after(BoltSystems::CellCollision)` so bump grading evaluates it), `afterimage_spawn_phantom_bolt` (reads `BumpPerformed`; on Perfect grade against a `PhantomBreaker` entity: spawns phantom bolt via `Bolt::builder().extra().headless()`; re-uses `PhantomBolt`/`PhantomLifetime`/`PhantomOwner` from `effect_v3/effects/phantom_bolt/components`; phantom bolt lifetime tick delegated to `tick_phantom_lifetime` in `EffectV3Plugin`; cleanup via `CleanupOnExit::<NodeState>`). `PhantomBreaker` is a zero-field marker; `PhantomBreakerLifetime(pub f32)` is a separate component.

### Remaining
- Wave 5 runtime behaviour for all custom-system protocols except those listed as delivered: all 10 are now done. (debt_collector, iron_curtain, echo_strike, siphon, greed, reckless_dash, burnout, conductor, fission, afterimage are done.)
