...
- Wave 5 runtime behaviour — Conductor (2026-04-20, branch `feature/protocol-conductor`): `conductor_swap_on_perfect_bump` in FixedUpdate swaps `PrimaryBolt`/`ExtraBolt` markers and `BoundEffects`/`StagedEffects` components between the bumped `ExtraBolt` and the current `PrimaryBolt` on every Perfect bump. Single system; no grace window; no bolt-lost suppression (dropped in design pivot — any bolt lost still costs a life). `ConductorConfig.primary_swap_window` field is vestigial (retained for RON symmetry). Ordering: `.after(BreakerSystems::GradeBump)`, `.before(EffectV3Systems::Bridge)` — ensures the swap precedes bridge systems that walk the effect tree on the newly promoted bolt.
