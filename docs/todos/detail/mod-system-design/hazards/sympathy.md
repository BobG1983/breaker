...
**Sends**: `HealDealt<Cell>` (with `HealCap::Starting`) via `apply_damage::<Cell>`'s post-damage hook in the cells domain.
...
5. Sends `HealDealt<Cell>` for each adjacent cell (ring 1: `damage * heal_percent / 100.0`, ring N: attenuating). Cap variant: `HealCap::Starting` (neighbour heals must not push past pristine HP).
...
| `shared::death_pipeline` | Heals adjacent cells | `HealDealt<Cell>` with `HealCap::Starting` (send) |
...
1. **Adjacent cells healed on damage at stack=1**
   ...
   - Then: B receives `HealDealt::<Cell> { amount: 25.0, cap: HealCap::Starting, .. }`, C receives `HealDealt::<Cell> { amount: 25.0, cap: HealCap::Starting, .. }`. A takes full 100 damage (no reduction).
...
2. **Heal percentage scales with stack at stack=3**
   ...
   - When: `sympathy_heal_adjacent` runs
   - Then: B, C, D each receive `HealDealt::<Cell> { amount: 28.0, cap: HealCap::Starting, .. }` (80 * 0.35)
...
- **Sympathy + Cascade interaction**: Cascade heals adjacent cells when a cell is DESTROYED. Sympathy heals adjacent cells when a cell is DAMAGED (even if not killed). Both produce `HealDealt<Cell>` messages. They are additive -- both effects apply. A cell taking damage heals its neighbors (Sympathy), and if it dies, Cascade heals them again.
...
- **Heal past max HP**: Sympathy sends `HealDealt<Cell>` with `HealCap::Starting`, so `apply_heal<Cell>` caps each heal at `Hp.starting`. Volatility's 2× regen limit is enforced by the Volatility sender itself (it does not send a heal if already at `starting * 2`), not by `HealCap::Max` — so there is no conflict between the two caps.
