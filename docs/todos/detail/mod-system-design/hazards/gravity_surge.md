...
**Reads**: `Destroyed<Cell>` (or equivalent cell death event with position)
...
- **Echo Cells**: When ghost cells (from Echo Cells hazard) are destroyed, they should also spawn gravity wells. The system reads `Destroyed<Cell>` generically -- it doesn't distinguish cell types.
