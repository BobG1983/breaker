...
5. **Echo Strike migration**. Move the existing Echo Strike emit system (today likely in `mutators/protocols/echo_strike/`) into the `MutateDamage` chain at the correct position (earliest — see the §Ordering rationale section above).
