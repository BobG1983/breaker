...
- **Wave 2 (Scaffold)**: All types, traits, components, resources, stubs, and plugin wiring must exist. In particular, `EffectV3Systems::Bridge`, `EffectV3Systems::Tick`, `EffectV3Systems::Conditions`, and `EffectV3Systems::Reset` system sets must exist in `src/effect_v3/sets.rs`.
...
### Failing Tests
- `src/effect_v3/effects/speed_boost/tests.rs` — passive fire/reverse/aggregate tests
- `src/effect_v3/effects/size_boost/tests.rs` — passive fire/reverse/aggregate tests
- `src/effect_v3/effects/damage_boost/tests.rs` — passive fire/reverse/aggregate tests
- `src/effect_v3/effects/bump_force/tests.rs` — passive fire/reverse/aggregate tests
- `src/effect_v3/effects/quick_stop/tests.rs` — passive fire/reverse/aggregate tests
- `src/effect_v3/effects/vulnerable/tests.rs` — passive fire/reverse/aggregate tests
- `src/effect_v3/effects/piercing/tests.rs` — passive fire/reverse/aggregate + PiercingRemaining tests
- `src/effect_v3/effects/ramping_damage/tests.rs` — passive fire/reverse/aggregate + RampingDamageAccumulator tests
- `src/effect_v3/effects/lose_life/tests.rs` — fire sends DamageDealt<Breaker> test
- `src/effect_v3/effects/time_penalty/tests.rs` — fire sends ApplyTimePenalty test
- `src/effect_v3/effects/die/tests.rs` — fire sends KillYourself<T> test
- `src/effect_v3/effects/spawn_bolts/tests.rs` — fire spawns bolts tests
- `src/effect_v3/effects/chain_bolt/tests.rs` — fire spawns tethered bolt test
- `src/effect_v3/effects/mirror_protocol/tests.rs` — fire spawns mirrored bolt tests
- `src/effect_v3/effects/random_effect/tests.rs` — fire delegates to random selection tests
- `src/effect_v3/effects/explode/tests.rs` — fire damages cells in range test
- `src/effect_v3/effects/piercing_beam/tests.rs` — fire damages cells in beam test
- `src/effect_v3/effects/shockwave/tests.rs` — fire spawns shockwave entity + tick/sync/damage/despawn tests
- `src/effect_v3/effects/chain_lightning/tests.rs` — fire spawns chain entity + tick state machine tests
- `src/effect_v3/effects/anchor/tests.rs` — fire/reverse + tick plant/unplant state machine tests
- `src/effect_v3/effects/attraction/tests.rs` — fire/reverse + apply attraction force tests
- `src/effect_v3/effects/pulse/tests.rs` — fire/reverse + tick periodic emission tests
- `src/effect_v3/effects/shield/tests.rs` — fire/reverse + tick countdown + reflection cost tests
- `src/effect_v3/effects/second_wind/tests.rs` — fire/reverse tests
- `src/effect_v3/effects/flash_step/tests.rs` — fire/reverse toggle tests
- `src/effect_v3/effects/circuit_breaker/tests.rs` — fire counter + reward tests
- `src/effect_v3/effects/entropy_engine/tests.rs` — fire counter + random pool tests
- `src/effect_v3/effects/gravity_well/tests.rs` — fire spawns well entity + tick force + despawn tests
- `src/effect_v3/effects/phantom_bolt/tests.rs` — fire spawns phantom entity + tick lifetime + despawn tests (note: directory name is `phantom_bolt` but the config struct is `SpawnPhantomConfig`)
- `src/effect_v3/effects/tether_beam/tests.rs` — fire spawns beam entity + tick damage + cleanup tests
- `src/effect_v3/effects/ramping_damage/tests.rs` — reset accumulator on node start test (shares file with passive tests)
- `src/effect_v3/conditions/evaluate_conditions/tests.rs` — condition transition fire/reverse tests
- `src/effect_v3/conditions/node_active.rs` — NodeActive evaluator tests (inline)
- `src/effect_v3/conditions/shield_active.rs` — ShieldActive evaluator tests (inline)
- `src/effect_v3/conditions/combo_active.rs` — ComboActive evaluator tests (inline)
- `src/effect_v3/dispatch/fire_dispatch.rs` — 30 match arm dispatch tests
- `src/effect_v3/dispatch/reverse_dispatch.rs` — 16 match arm dispatch tests
...
**1. SpeedBoostConfig**
- File: `src/effect_v3/effects/speed_boost/config.rs`
...
- Component: `PiercingRemaining(pub u32)` in `src/effect_v3/effects/piercing/components.rs`

**8. RampingDamageConfig**
- File: `src/effect_v3/effects/ramping_damage/config.rs`
...
- Component: `RampingDamageAccumulator(pub OrderedFloat<f32>)` in `src/effect_v3/effects/ramping_damage/components.rs`
...
**9. LoseLifeConfig**
- File: `src/effect_v3/effects/lose_life/config.rs`
...
- File: `src/effect_v3/effects/time_penalty/config.rs`
...
**11. DieConfig**
- File: `src/effect_v3/effects/die/config.rs`
...
- File: `src/effect_v3/effects/random_effect/config.rs`
...
**13. FlashStepConfig**
- File: `src/effect_v3/effects/flash_step/config.rs`
...
- Component: `FlashStepActive` (marker, `#[derive(Component)]`) in `src/effect_v3/effects/flash_step/components.rs`
- No `register` override
...
**14. ShieldConfig**
- File: `src/effect_v3/effects/shield/config.rs`
...
- `register`: register `tick_shield_duration` in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/shield/components.rs`:
  - `ShieldWall` — marker
...
**15. SecondWindConfig**
- File: `src/effect_v3/effects/second_wind/config.rs`
...
- Components in `src/effect_v3/effects/second_wind/components.rs`:
  - `SecondWindWall` — marker
  - `SecondWindOwner(pub Entity)`

**16. PulseConfig**
- File: `src/effect_v3/effects/pulse/config.rs`
...
- `register`: register `tick_pulse` in `EffectV3Systems::Tick`
- Component in `src/effect_v3/effects/pulse/components.rs`:
...
**17. AnchorConfig**
- File: `src/effect_v3/effects/anchor/config.rs`
...
- `register`: register `tick_anchor` in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/anchor/components.rs`:
...
**18. AttractionConfig**
- File: `src/effect_v3/effects/attraction/config.rs`
...
- `register`: register `apply_attraction` in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/attraction/components.rs`:
...
- File: `src/effect_v3/effects/circuit_breaker/config.rs`
...
- Component in `src/effect_v3/effects/circuit_breaker/components.rs`:
...
**20. EntropyConfig**
- File: `src/effect_v3/effects/entropy_engine/config.rs`
...
- Component in `src/effect_v3/effects/entropy_engine/components.rs`:
...
**21. ShockwaveConfig**
- File: `src/effect_v3/effects/shockwave/config.rs`
...
- `register`: register the 4 chained shockwave systems in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/shockwave/components.rs`:
...
**22. ExplodeConfig**
- File: `src/effect_v3/effects/explode/config.rs`
...
- File: `src/effect_v3/effects/chain_lightning/config.rs`
...
- `register`: register `tick_chain_lightning` in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/chain_lightning/components.rs`:
...
- File: `src/effect_v3/effects/piercing_beam/config.rs`
...
**25. SpawnBoltsConfig**
- File: `src/effect_v3/effects/spawn_bolts/config.rs`
...
- File: `src/effect_v3/effects/phantom_bolt/config.rs`
...
- `register`: register `tick_phantom_lifetime` in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/phantom_bolt/components.rs`:
  - `PhantomBolt` — marker
  - `PhantomLifetime(pub f32)`
  - `PhantomOwner(pub Entity)`

**27. ChainBoltConfig**
- File: `src/effect_v3/effects/chain_bolt/config.rs`
...
**28. MirrorConfig**
- File: `src/effect_v3/effects/mirror_protocol/config.rs`
...
**29. TetherBeamConfig**
- File: `src/effect_v3/effects/tether_beam/config.rs`
...
- `register`: register `tick_tether_beam_damage` and `cleanup_tether_beams` (chained) in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/tether_beam/components.rs`:
  - `TetherBeamSource { bolt_a: Entity, bolt_b: Entity }`
  - `TetherBeamDamage(pub f32)`

**30. GravityWellConfig**
- File: `src/effect_v3/effects/gravity_well/config.rs`
...
- `register`: register `tick_gravity_wells` and `despawn_expired_wells` (chained) in `EffectV3Systems::Tick`
- Components in `src/effect_v3/effects/gravity_well/components.rs`:
...
**EffectSourceChip**
- File: `src/effect_v3/components/effect_source_chip.rs`
...
All tick systems run in `FixedUpdate`, in `EffectV3Systems::Tick`, with `run_if(in_state(NodeState::Playing))`. The `EffectV3Systems::Tick` set is ordered after `EffectV3Systems::Bridge` by the set-level ordering constraint established in wave 2. Individual tick systems do NOT need additional `.after()` calls beyond those required for chaining within the same effect (e.g., the 4 shockwave systems are chained together).

**tick_shockwave**
- File: `src/effect_v3/effects/shockwave/systems.rs`
...
- File: `src/effect_v3/effects/chain_lightning/systems.rs`
...
**tick_anchor**
- File: `src/effect_v3/effects/anchor/systems.rs`
...
**apply_attraction**
- File: `src/effect_v3/effects/attraction/systems.rs`
...
**tick_pulse**
- File: `src/effect_v3/effects/pulse/systems.rs`
...
- File: `src/effect_v3/effects/shield/systems.rs`
...
- Schedule: in `EffectV3Systems::Tick` — no additional `.after()` needed beyond set-level ordering. The set runs after Bridge, which runs after collision systems.

**tick_phantom_lifetime**
- File: `src/effect_v3/effects/phantom_bolt/systems.rs`
...
- File: `src/effect_v3/effects/tether_beam/systems.rs`
...
**tick_gravity_wells**
- File: `src/effect_v3/effects/gravity_well/systems.rs`
...
#### GROUP I: Reset Systems (EffectV3Systems::Reset)

**reset_ramping_damage**
- File: `src/effect_v3/effects/ramping_damage/systems.rs`
...
**evaluate_conditions**
- File: `src/effect_v3/conditions/evaluate_conditions.rs`
- Schedule: `FixedUpdate`, in `EffectV3Systems::Conditions`, after `EffectV3Systems::Tick`
- Logic:
...
- Follow the folder-per-effect pattern: `src/effect_v3/effects/<name>/` with `mod.rs`, `config.rs`, optionally `components.rs`, `systems.rs`
...
**FixedUpdate, EffectV3Systems::Tick** (all with `run_if(in_state(NodeState::Playing))`):
...
**FixedUpdate, EffectV3Systems::Conditions** (after EffectV3Systems::Tick):
- `evaluate_conditions`

**OnEnter(NodeState::Playing), EffectV3Systems::Reset**:
- `reset_ramping_damage`

### Wiring

Each effect's `register` method (called from `EffectV3Plugin::build`) handles its own system registration. The plugin calls `register` for every config type:

```rust
// In EffectV3Plugin::build (or equivalent)
...
app.add_systems(FixedUpdate,
    evaluate_conditions
        .in_set(EffectV3Systems::Conditions)
        .after(EffectV3Systems::Tick)
);
```

Module declarations in `src/effect_v3/effects/mod.rs`:
```rust
...
Module declarations in `src/effect_v3/conditions/mod.rs`:
```rust
...
### Constraints
- Do NOT modify: `src/effect_v3/types/` (enums defined in prior waves), `src/effect_v3/walking/` (tree walker from prior wave), `src/effect_v3/storage/` (BoundEffects/StagedEffects from prior wave), `src/effect_v3/commands/` (fire_effect/reverse_effect from prior wave), `src/effect_v3/stacking/` (EffectStack from prior wave)
- Do NOT modify: any domain outside `src/effect_v3/` (bolt, breaker, cells, run, etc.)
- Do NOT modify: `src/effect_v3/triggers/` (bridges from Wave 5)
