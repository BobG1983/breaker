...
2. Snapshot damage as `damage_mult * base_damage * damage_boosts`.
...
6. If `arcs > 1`, spawn a `ChainLightningChain` entity with `remaining_jumps: arcs - 1`, the snapshotted damage, a `hit_set` containing the first target, `state: Idle`, `range`, `arc_speed`, and `source_pos` set to the first target's position.
...
# Source Location
`src/effect/configs/chain_lightning.rs`
