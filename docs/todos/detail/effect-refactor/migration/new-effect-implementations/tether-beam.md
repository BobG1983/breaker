...
2. If `chain` is `false`:
   ...
   c. Spawn a tether beam entity with `TetherBeamSource` tracking the source bolt and the new bolt, and `TetherBeamDamage(damage_mult * base_damage)`.
3. If `chain` is `true`:
   a. Find the nearest other bolt entity to the source bolt.
   b. If found, spawn a tether beam entity with `TetherBeamSource` tracking the source bolt and the nearest bolt, and `TetherBeamDamage(damage_mult * base_damage)`.
   c. If no other bolt is found, do nothing.
4. Fire does NOT tick the beam -- `tick_tether_beam_damage` does.

# Reverse
Not reversible.

# Source Location
`src/effect/configs/tether_beam.rs`
