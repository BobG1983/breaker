...
| `deadline-integration-tests.md` | Integration tests for Deadline's code-driven form from TODO #7 — threshold crossing fires boosts on all bolts, late-spawn inheritance, node-exit reversal, stack-with-chip aggregation. |
...
- `end-to-end-integration-tests.md` → TODO #1 (damage crate), #2 (mutators domain), #7 (dispatcher + code-driven protocols)
- `deadline-integration-tests.md` → TODO #7 (Deadline's code-driven form)
- `diffusion-diamond-topology.md` → TODO #1 (Diffusion becomes a `MessageMutator<DamageDealt<Cell>>` post-crate)
- `ron-asset-tests-structural-only.md` → rides with TODO #10 (RON tuning sweep), which is where most value-pinned assertions currently live
