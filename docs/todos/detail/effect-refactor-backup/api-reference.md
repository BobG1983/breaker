...
| `Spawned` | `Spawned(EntityKind, Fire(...))` | `.spawned(type).fire(...)` | Entity add | Never | No | No |
...
|---|---|---|---|
| `Spawned(EntityKind)` | Bridge | (implicit target) | **NEW** — fires on `Added<Bolt/Cell/Wall/Breaker>` |
...
|---|---|---|---|
| `Stamp` | `Stamp(Target, tree)` | `.stamp(target)` | Stamp subtree onto entity type at load time. Sets `This` for the subtree. Adds to BoundEffects (permanent). |

`Stamp` is a **chip/breaker definition wrapper** — it tells the loader which entity type to stamp the subtree onto. Required at root of every `effects: []` entry.

Entity targets for Stamp:

| Target | Meaning |
|---|---|
| `Bolt` | Stamp onto bolt entities |
| `Breaker` | Stamp onto breaker entities |
| `Cell` | Stamp onto cell entities |
| `Wall` | Stamp onto wall entities |
| `ActiveBolts` | Stamp onto all existing bolt entities (point-in-time snapshot). Replaces `AllBolts`. |
...
`On` is ONLY used inside a tree to redirect Fire/Stamp/Route to a **non-This** target:
...
| `Route` | `Route(inner_tree)` | `.route(inner_tree)` | StagedEffects | One-shot arm a tree on the `On` target — consumed when triggered |
...
| `On(target: Bolt, then: [...])` | `Stamp(Bolt, ...)` (chip-level routing) |
| `On(target: Breaker, then: [...])` | `Stamp(Breaker, ...)` (chip-level routing) |
| `On(target: AllBolts, then: [...])` | `Stamp(ActiveBolts, ...)` (chip-level routing) |
