...
## 2. What `route_effects` Is

There is no function or system named `route_effects`. The plan doc uses this as a placeholder name. The actual mechanism is the `TransferCommand` struct.
...
Called via `commands.route_effect(entity, chip_name, children, permanent, context)`.
...
**Callers of `route_effect` / `TransferCommand`:**

- `dispatch_chip_effects` → `dispatch_children` → for non-Do, non-On nodes, calls `commands.route_effect(entity, chip_name, vec![node], true, TriggerContext::default())`
- `DispatchInitialEffects::apply` → `TransferCommand::apply` (for Breaker and Bolt targets at init time)
- `walk_staged_node` in the evaluate system does NOT call `route_effect` directly — it calls `commands.queue(ResolveOnCommand)` which internally chains to `TransferCommand`
...
The plan doc's question: "Need to verify pre-desugaring indexes are accessible in `route_effects`."
