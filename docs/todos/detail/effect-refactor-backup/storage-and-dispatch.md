...
Permanent effect trees. Populated by Stamp at load time and Stamp at runtime. Never consumed — When entries re-arm after firing.
...
**Route:**
```
BoundEffects (bolt): Impacted(Cell) -> [On(ImpactTarget::Impactee, Route(When(Died, Fire(Explode))))]

Bolt impacts cell:
  → Route fires: insert into cell's StagedEffects:
    StagedEffects (cell): Died -> [Fire(Explode)]

Cell dies:
  → execute Fire(Explode)
  → remove entry from StagedEffects (consumed)
  → bolt must impact again to re-route
```
...
struct OnSpawnEffectRegistry {
    /// EntityKind -> trees to stamp/route onto new entities of that type.
...
- Execution rules for inner trees:
  ...
  - `On(participant, terminal)` → resolve participant from TriggerContext:
    ...
    - `Route(tree)` → insert into participant's StagedEffects (one-shot)
...
| `route_effect(entity, source, tree)` | Add tree to entity's StagedEffects | StagedEffects |
...
fn walk_effects(
    ...
    commands: &mut Commands,    // defer fire/stamp/route via EffectCommandsExt
) {
    if context.depth >= MAX_DISPATCH_DEPTH {
...
Same-entity StagedEffects mutations (drain matching entries) happen inline during the walk. Cross-entity operations (Stamp/Route onto another entity) and effect execution (Fire/Reverse) are deferred via command extensions. Bevy applies commands at schedule flush points, at which point the commands have exclusive world access.
...
- No aliased mutable borrows (Stamp/Route target other entities via deferred commands)
