...
## Summary
Comprehensive effect system refactor: unified RON/builder vocabulary, new wrappers (`When`/`During`/`Until`/`Spawned`), new targets (`EveryBolt`/`ActiveBolts`/`PrimaryBolts`/`ExtraBolts`), named trigger participants, `Stamp`/`Route` terminals, typestate builder with `Reversible` trait enforcement, unified damage/death pipeline. Clean room implementation in `src/new_effect/`, swap when complete.
...
| [command-extensions.md](command-extensions.md) | EffectCommandsExt behavioral spec: fire_effect, reverse_effect, stamp_effect, route_effect, remove_source, dispatch_fire/dispatch_reverse |
...
- **Unified vocabulary**: Stamp/When/During/Until/Spawned/On/Fire/Route/Sequence
- **Stamp** = definition-level routing to BoundEffects (permanent). Required root of every `effects: []` entry. Also runtime terminal to BoundEffects (permanent).
- **Route** = runtime terminal to StagedEffects (one-shot).
...
- **Command extensions for dispatch**: walk_effects defers fire/stamp/route via EffectCommandsExt on Commands. Bridge systems are regular Bevy systems (not exclusive).
...
- **Detach on route** — no cross-entity cleanup tracking
...
```
src/new_effect/
  builder/        # Stamp, Route, Fire, EffectDef, EffectTree
...
- `Spawned(EntityKind)` trigger (implicit target)
...
- `Stamp` definition routing + runtime permanent terminal, `Route` runtime one-shot terminal
...
| **A5**: Entry point naming | **EffectDef::stamp()** | Stamp is definition-level (permanent add to BoundEffects). Route is a runtime terminal (one-shot add to StagedEffects). |
...
- **builder-design.md**: Add RandomEffect(Vec<(f32, Box<EffectType>)>) to EffectType enum. Update EntropyConfig pool type. Fix Vulnerable to bare f32. Add Sequence/Spawned to loader pseudocode. Fix EventReader → MessageReader.
