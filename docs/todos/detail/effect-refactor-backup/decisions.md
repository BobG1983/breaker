...
**Desugar at load time.** `Stamp(EveryBolt, tree)` expands to: (1) stamp tree onto all existing bolts via ActiveBolts query, (2) register tree in `OnSpawnEffectRegistry` resource for future bolts. OnSpawnEffectRegistry is a global `Resource<OnSpawnEffectRegistry>` holding `HashMap<EntityKind, Vec<(SourceId, ValidTree)>>`.
...
**4 standard systems in PostFixedUpdate** (not Bevy Observers). One per entity type: `bridge_bolt_added`, `bridge_cell_added`, `bridge_wall_added`, `bridge_breaker_added`. Each queries `Added<Bolt/Cell/Wall/Breaker>`, reads OnSpawnEffectRegistry for matching entries, stamps/routes trees onto the new entity's BoundEffects/StagedEffects.
...
## 12. Route/Stamp tree ownership
**Detach on route.** Once routed/stamped onto another entity, the tree has no link back to the source. Unequipping the chip removes the bolt's BoundEffects entries (stops future routes) but doesn't touch entities that already received trees.

## 13. Chip loading -> Route processing
**Equip command processes Stamps.** Same timing as today. The chip equip command reads each ValidDef, matches on Target, and stamps the tree into the target entity's BoundEffects with the chip's SourceId. EveryBolt desugars here: stamp existing + register in OnSpawnEffectRegistry.
