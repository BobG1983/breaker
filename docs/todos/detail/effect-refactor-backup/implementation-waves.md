...
### 1c. Target + ParticipantTarget
New file: `new_effect/tree/targets.rs`

```rust
enum Target { Bolt, Breaker, Cell, Wall, ActiveBolts, EveryBolt, PrimaryBolts, ExtraBolts, ... }
enum ParticipantTarget { Bump(BumpTarget), Impact(ImpactTarget), Death(DeathTarget), BoltLost(BoltLostTarget) }
```

Reference: [builder-design.md](builder-design.md) Target + ParticipantTarget sections
...
```rust
struct RawDef { target: RawTarget, tree: RawTree }
enum RawTree { Fire, Sequence, When, Once, During, Until, Spawned, On }
enum RawTerminal { Fire, Stamp, Route }
...
struct OnSpawnEffectRegistry { entries: HashMap<EntityKind, Vec<SpawnedEntry>> }  // EntityKind impls Hash+Eq
```
...
### 2a. EffectDef::stamp() + StampContext
New file: `new_effect/builder/stamp.rs`

The definition-level entry point. Returns StampContext which chains into inner tree building.

Reference: [builder-design.md](builder-design.md) StampContext section, [examples.md](examples.md) all examples

### 2b. EffectTree entry points
New file: `new_effect/builder/tree.rs`

Inner tree builder for Route/Stamp payloads. `EffectTree::when()`, `EffectTree::fire()`, etc.
...
Terminal builders: `.fire()`, `.stamp()`, `.route()`, `.sequence()`.
...
The shared tree-walking function all bridge systems call. StagedEffects first, then BoundEffects. Handles When (arm into StagedEffects), Once (arm + mark for removal), Fire/Stamp/Route (deferred via Commands), Sequence (ordered), On (redirect to participant entity).
...
Same-entity StagedEffects mutations (drain matching entries) happen inline. Cross-entity Stamp/Route and effect Fire/Reverse are deferred via `EffectCommandsExt` on Commands. Bevy applies commands at schedule flush points.
...
### 4d. Stamp processing / equip command (parallel with 4b)
New file: `new_effect/dispatch/stamp.rs`

Processes ValidDef list at chip equip time. Resolves Target, stamps into BoundEffects, desugars EveryBolt.

```rust
fn process_stamps(defs: &[ValidDef], entity_map: &StampEntityMap, ...)
```
...
trait EffectCommandsExt {
    ...
    fn route_effect(entity, source, tree);
...
> Unified `Hp { current: f32, starting: f32, max: Option<f32> }` component used by all damageable entity types (cells, breakers with health). Replaces domain-specific CellHealth.
...
- Stamp/Route semantics

**`docs/design/`:**
- Effect system vocabulary reference (Stamp, When, During, Until, Spawned, On, Fire, Stamp, Route, Sequence)
