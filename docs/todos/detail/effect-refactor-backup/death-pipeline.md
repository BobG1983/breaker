...
// In explode fire() (from powder keg Route):
...
Stamp(Cell, When(Died, Fire(Explode(range: 48.0, damage: 10.0))))
...
// Powder keg: "when I hit a cell, route 'when you die, explode' onto it"
...
Stamp(Wall, When(Impacted(Bolt), Fire(Die)))

// Generic kill reward: "every kill boosts my speed"
Stamp(Bolt, When(Killed(Any), Fire(SpeedBoost(1.1))))
```
...
```ron
// Before                              // After
On(target: Bolt, then: [...])      →  Stamp(Bolt, ...)
On(target: Breaker, then: [...])   →  Stamp(Breaker, ...)
Do(...)                            →  Fire(...)
