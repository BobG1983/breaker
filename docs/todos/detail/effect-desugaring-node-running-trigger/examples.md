...
```
```ron
Route(Bolt, Fire(DamageBoost(multiplier: 3.0)))
```
...
```
```ron
Route(Bolt, When(Impacted(Wall), Fire(Shockwave(base_range: 64.0, speed: 500.0))))
```
...
```
```ron
Route(Bolt, When(PerfectBumped, Until(TimeExpires(3.0), Fire(SpeedBoost(multiplier: 1.5)))))
```
...
```
```ron
Route(Bolt, Spawned(Bolt, Fire(Piercing(count: 3))))
```
...
```
```ron
Route(Bolt, Until(Died, When(PerfectBumped, Fire(Explode(range: 50.0, damage: 10.0)))))
```
...
```
```ron
Route(Bolt, When(Killed(Cell), Fire(SpeedBoost(multiplier: 1.3))))
```
...
```
```ron
Route(Breaker, When(PerfectBumped, On(BumpTarget::Bolt, Fire(FlashStep))))
```
...
| `Transfer(tree)` | StagedEffects | One-shot | No -- consumed on match |
