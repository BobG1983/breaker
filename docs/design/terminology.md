...
| **HighlightTracker** | Per-node intermediate state resource used for highlight detection. Tracks consecutive perfect bumps, node-level bolts lost, cell destruction timestamps, and node start time. Reset on each `OnEnter(Playing)` (new node). | `HighlightTracker`, `reset_highlight_tracker` |
| **HighlightKind** | Enum of memorable run moment categories: `ClutchClear` (cleared with < 3s remaining), `MassDestruction` (10+ cells in a 1s window), `PerfectStreak` (5+ consecutive perfect bumps), `FastClear` (cleared in < 50% of time), `FirstEvolution`, `NoDamageNode` | `HighlightKind`, `RunHighlight` |
| **RunHighlight** | A single recorded highlight moment. Has `kind: HighlightKind`, `node_index: u32`, and `value: f32` (context-dependent — seconds remaining for ClutchClear, streak count for PerfectStreak, etc.). Up to 3 are displayed on the run-end screen. | `RunHighlight`, `RunStats::highlights` |
