...
| **CellBehavior** | An enum variant attached to a `CellTypeDefinition` that activates runtime behavior at spawn. Current variants: `Regen { rate }`, `Guarded(GuardedBehavior)`, `Volatile { damage, radius }`. | `CellBehavior`, `CellTypeDefinition.behaviors` |
