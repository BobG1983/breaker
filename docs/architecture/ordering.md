...
| `BoltSystems::SyncSpeedToStack` | `bolt/sets.rs` | `sync_bolt_speed_to_stack` — re-applies `apply_velocity_formula` after `EffectV3Systems::Death` + `ApplyDeferred` barrier so death-trigger speed boosts land on the same tick. Ordered after `EffectV3Systems::Death`; scenario-runner invariant checkers order `.after(BoltSystems::SyncSpeedToStack)` to observe post-sync velocity. |
            ...
            (handle_kill::<Bolt>, handle_kill::<Wall>, handle_kill::<Breaker>, handle_kill::<Salvo> — crate-internal; handle_breaker_death from RunPlugin)
              <- EffectV3Systems::Death .after(DmgSystems::ApplyKill)  [effect_v3 death bridges]
                 (on_cell_destroyed, on_bolt_destroyed, on_wall_destroyed, on_breaker_destroyed)
                   <- ApplyDeferred .after(EffectV3Systems::Death)
                                   .before(BoltSystems::SyncSpeedToStack)
                      [Bevy barrier — flushes deferred FireEffectCommand insertions (SpeedBoost stack
                       entries) before sync reads the stack]
                        <- sync_bolt_speed_to_stack .after(ApplyDeferred above)
                           BoltSystems::SyncSpeedToStack              [bolt domain]
                           [re-applies apply_velocity_formula so death-trigger speed boosts take
                            effect on the same tick the cell died, not one tick later]
