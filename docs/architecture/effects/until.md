...
The same applies to `Once(_, Until(...))` and `When(_, Until(...))` — when the outer gate matches, `evaluate_when` / `evaluate_once` do **not** stage the inner `Until`. Instead, they delegate directly to `evaluate_until`, which calls `ensure_until_bound` to self-bind the Until into `BoundEffects`. Staging was rejected for `Until` because a staged `Until(TimeExpires(d), ...)` would be gate-filtered by the bridge against `TimeExpires(d)` — which can only fire after the timer is armed, which only happens inside `UntilEvaluateCommand::apply`. The direct-delegate path avoids this Catch-22: the Until binds itself during the arming tick, the timer is armed in the same command flush, and the bridge can find the bound entry when the timer expires.

For `Once(_, Until(...))` specifically, the outer `Once` queues its own `RemoveEffectCommand` before delegating to `evaluate_until`, so the outer's `BoundEffects` slot is cleared before `ensure_until_bound` writes the new `Until(...)` entry under the same source name.
