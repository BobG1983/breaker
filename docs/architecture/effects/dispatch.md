...
`On(AllCells, children)` becomes `Once(When(NodeStart, On(AllCells, permanent: true, children)))`
...
3. The `Once` is consumed — this only happens once, not on every future node start
...
Once(When(trigger: NodeStart, then: [
    On(target: AllCells, then: [When(trigger: Died, then: [Do(Shockwave(...))])])
]))
// ...and pushes it to the primary bolt or breaker's BoundEffects
```
