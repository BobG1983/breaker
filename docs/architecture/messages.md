...
| `Destroyed<Cell> { victim, killer, victim_pos, killer_pos, _marker }` | rantzsoft_dmg crate-internal (handle_kill::<Cell>) | run/node (track_node_completion), effect (on_cell_destroyed), mutators/protocols/fission (fission_on_cell_destroyed — increments FissionCounter, spawns bolt clone on every Nth kill), mutators/protocols/echo_strike (echo_strike_cleanup_destroyed_echoes — removes dead cells from EchoNetwork deques) |
...
| `BumpPerformed { grade, bolt, breaker }` | breaker, mutators/protocols/afterimage::afterimage_check_phantom_bounce (synthetic phantom bump — msg.breaker carries PhantomBreaker, not Breaker) | breaker (spawn_bump_grade_text, perfect_bump_dash_cancel), effect (bridge_bump), mutators/protocols/reckless_dash (reckless_dash_on_bump), mutators/protocols/conductor (conductor_swap_on_perfect_bump — Perfect grade only, ExtraBolt target only), mutators/protocols/afterimage::afterimage_spawn_phantom_bolt (reads Perfect-grade phantom bumps to spawn phantom-bolt clones) |
...
| `ReverseTimePenalty { seconds }` | effect/effects/time_penalty (reverse), mutators/protocols/siphon (siphon_on_cell_destroyed) | run/node (reverse_time_penalty) |
