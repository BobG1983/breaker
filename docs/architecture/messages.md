...
| `DamageDealt<Cell> { dealer, target, amount, source_chip }` | bolt (bolt_cell_collision), cells (tick_survival_timer — self-destruct), cells (salvo_cell_collision), effect/effects (shockwave, explode, pulse, chain_lightning, piercing_beam, tether_beam), protocol/protocols/debt_collector (debt_collector_on_impact), protocol/protocols/iron_curtain (iron_curtain_on_bolt_lost), protocol/protocols/echo_strike (echo_strike_on_impact) | cells (check_armor_direction — mutating interceptor: drains, filters blocked hits, re-extends before apply_damage sees the queue), cells (apply_damage_to_cells — owns the Cell damage application with Diffusion BFS redistribution; replaces the generic `apply_damage::<Cell>`) |
...
| `HealDealt<Cell> { healer, target, amount, source, cap }` | hazard (`volatility_grow_cells`, `cascade_heal_on_death`, `renewal_regrow`, `momentum_heal_on_nonlethal`, `sympathy_heal_adjacent`) | shared/death_pipeline (apply_heal::<Cell>) |
