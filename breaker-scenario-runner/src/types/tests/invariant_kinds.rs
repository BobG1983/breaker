...
fn invariant_kind_second_wind_wall_at_most_one_parses() {
    let result: InvariantKind = ron::de::from_str("SecondWindWallAtMostOne")
        .expect("SecondWindWallAtMostOne should parse");
    assert_eq!(result, InvariantKind::SecondWindWallAtMostOne);
}

#[test]
fn invariant_kind_shield_charges_consistent_parses() {
    let result: InvariantKind = ron::de::from_str("ShieldChargesConsistent")
        .expect("ShieldChargesConsistent should parse");
    assert_eq!(result, InvariantKind::ShieldChargesConsistent);
}

#[test]
fn invariant_kind_pulse_ring_accumulation_parses() {
    let result: InvariantKind = ron::de::from_str("PulseRingAccumulation")
        .expect("PulseRingAccumulation should parse");
