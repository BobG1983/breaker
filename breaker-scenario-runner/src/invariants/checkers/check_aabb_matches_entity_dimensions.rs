...
/// `Vec2::new(BreakerWidth.half_width() * scale, BreakerHeight.half_height() * scale)`
/// where `scale` comes from <code>[EntityScale].0</code> (defaulting to `1.0` if absent).
///
...
pub fn check_aabb_matches_entity_dimensions(
    ...
    for (entity, aabb, width, height, entity_scale) in &breakers {
        let scale = entity_scale.map_or(1.0, |s| s.0);
        let expected = Vec2::new(width.half_width() * scale, height.half_height() * scale);
        let actual = aabb.half_extents;
        if (actual - expected).abs().max_element() > AABB_EPSILON {
            log.0.push(ViolationEntry {
                frame: frame.0,
...
mod tests {
    ...
    /// Behavior 10: No violation when breaker `Aabb2D` `half_extents` match width/height with `EntityScale`.
    #[test]
    fn no_violation_when_breaker_aabb_matches_with_scale() {
        ...
        assert!(
            log.0.is_empty(),
            "no violation expected when breaker half_extents (80.0, 12.0) match scaled dimensions (40.0*2.0, 6.0*2.0)"
        );
    }
    ...
    /// Behavior 12: Violation fires when breaker `Aabb2D` `half_extents` do not match expected (with scale).
    #[test]
    fn violation_when_breaker_aabb_does_not_match_with_scale() {
        ...
        assert_eq!(
            log.0.len(),
            1,
            "expected 1 violation when breaker half_extents (40.0, 6.0) != expected scaled (80.0, 12.0)"
        );
        assert_eq!(
            log.0[0].invariant,
            InvariantKind::AabbMatchesEntityDimensions
