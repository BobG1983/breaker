...
//! `apply_vulnerable`. These tests guard against regressions where the
...
//! `vulnerable_stack/tests/`. These tests focus only on the path from
...
fn filtered_persistent_entry_applies_when_emission_source_matches() {
    ...
    enqueue(
        &mut app,
        mk_msg_with_source(None, target, Some(SourceId::from("emit:burnout")), 4.0),
    );
    tick(&mut app);
...
fn filtered_persistent_entry_skipped_when_emission_source_mismatches() {
    ...
    enqueue(
        &mut app,
        mk_msg_with_source(None, target, Some(SourceId::from("emit:lightning")), 4.0),
    );
    tick(&mut app);
...
fn filtered_persistent_entry_skipped_when_emission_source_is_none() {
    ...
    enqueue(&mut app, mk_msg_with_source(None, target, None, 4.0));
    tick(&mut app);
...
fn filtered_one_shot_consumed_only_when_emission_source_matches() {
    ...
    let drained = drain_messages(&mut app);
    assert_eq!(drained.len(), 1);
    assert_f32_eq(drained[0].amount, 12.0);
...
fn filterless_one_shot_drains_for_any_emission_source() {
    ...
    enqueue(
        &mut app,
        mk_msg_with_source(None, target, Some(SourceId::from("emit:any")), 5.0),
    );
    tick(&mut app);
